...
export default {
  ...
  methods: {
    ...
    loadMore () {
      if (this.nextMarker) {
        this.loading = true
        listObjects(this.currentBucket, this.currentPath, this.nextMarker).then((res) => {
          this.loading = false
          this.nextMarker = res.data.nextMarker
          for (let i = 0; i < res.data.objectList.length; i++) {
            this.datalist.push(res.data.objectList[i])
    ...
    listPath (dir) {
      ...
      this.loading = true
      let path = obj.path.substring(0, obj.path.length - 1)
      // console.log(path)
      ...
      listObjects(this.currentBucket, obj.path).then((res) => {
        this.loading = false
        obj.path = path + '/'
